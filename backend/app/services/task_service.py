...
class TaskService:
	...
	def _create_task_from_slack_sync(self, command: SlackCommandPayload) -> Dict:
		...
		if not assignee_member:
			raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Assignee email was not found in FlowSync')
	...
	def _create_task_sync(
		...
		*,
		...
		if create_github_issue:
			issue_data = self._maybe_create_github_issue(client, org_id, title, description, assigned_member.get('email'))
			if issue_data:
	...
	def _maybe_create_github_issue(self, client, org_id: str, title: str, description: str, assignee_email: Optional[str]) -> Optional[Dict]:
		integrations = self._get_org_integrations(client, org_id)
		repo = (integrations.get('github') or {}).get('repo')
