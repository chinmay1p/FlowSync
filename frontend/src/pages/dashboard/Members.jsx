		...
		<div className="grid gap-8 xl:grid-cols-[2fr_1fr]">
			<div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-xl">
				<div className="flex items-center justify-between">
					<h1 className="text-xl font-semibold text-slate-900">Organization members</h1>
						...
						<tbody>
							{members.map((member) => (
								<tr
									...
									className={`cursor-pointer border-b border-slate-100 text-sm ${
										member.uid === selectedMemberId ? 'bg-slate-50' : ''
									}`}
								>
									<td className="py-2 text-slate-900">{member.email || member.uid}</td>
				...
				{selectedMember ? (
					<>
						<div>
							...
							<h2 className="text-2xl font-semibold text-slate-900">{selectedMember.email || selectedMember.uid}</h2>
							<p className="text-xs text-slate-500">Org role · {selectedMember.role}</p>
						</div>
						<div className="space-y-3">
							{(memberTeamsMap[selectedMember.uid] || []).map((team) => (
								<div key={team.teamId} className="rounded-2xl border border-slate-100 px-4 py-3">
									<div className="flex items-center justify-between">
										<div>
											<p className="text-sm font-semibold text-slate-900">{team.teamName}</p>
											<p className="text-xs text-slate-500">Team ID · {team.teamId}</p>
										</div>
						...
						{admin && (
							<form onSubmit={handleAssign} className="rounded-2xl border border-dashed border-slate-300 p-4 space-y-3">
								<h3 className="text-sm font-semibold text-slate-600">Assign to team</h3>
								<select
									value={assignTeamId}
