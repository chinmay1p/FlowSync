	...
	return (
		<div className="grid gap-8 lg:grid-cols-[280px_1fr]">
			<div className="space-y-6">
							...
							<li key={team.teamId}>
								<button
									...
									className={`w-full rounded-2xl px-4 py-3 text-left text-sm font-semibold transition ${
										selectedTeamId === team.teamId ? 'bg-slate-900 text-white shadow-md' : 'bg-slate-50 text-slate-700 hover:bg-slate-100'
									}`}
								>
									{team.teamName}
									<span className="block text-xs font-normal text-slate-400">{team.members?.length || 0} members</span>
								</button>
							</li>
						))}
				...
				{admin && (
					<form onSubmit={handleCreateTeam} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-lg space-y-3">
						<h3 className="text-sm font-semibold text-slate-600">Create team</h3>
						<input
							type="text"
							value={newTeamName}
			...
			<div className="space-y-6">
				{selectedTeam ? (
					<div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-xl space-y-6">
						<div className="flex flex-wrap items-center justify-between gap-3">
							<div>
								...
								<h2 className="text-2xl font-semibold text-slate-900">{selectedTeam.teamName}</h2>
							</div>
						...
						<div className="space-y-3">
							...
							{teamMembers.map((member) => (
								<div key={member.uid} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-100 px-4 py-3">
									<div>
										<p className="text-sm font-semibold text-slate-900">{member.email || member.uid}</p>
										<p className="text-xs text-slate-500">{member.uid}</p>
									</div>
									<div className="flex items-center gap-2">
										<select
											...
											className="rounded-2xl border border-slate-200 px-3 py-1 text-xs font-semibold"
										>
						...
						{admin && (
							<form onSubmit={handleAssignMember} className="rounded-2xl border border-dashed border-slate-300 p-4 space-y-3">
								<h3 className="text-sm font-semibold text-slate-600">Assign member</h3>
								<select
									value={assignUser}
					...
					<div className="rounded-3xl border border-slate-200 bg-white p-6 text-sm text-slate-500 shadow-xl">
